...
import joblib
from util_mydep import mydep_dict
...
class KeywordExtractor():
    ...
    def extract_keywords(self, content):
        ...
        kw_dict = mydep_dict(content) # already sorted
        kws = " ".join(list(kw_dict.keys())[:self.n_kws] ) # get top n keywords
...
class KeywordCoverage():
    ...
    def build_io(self, contents_tokenized, contents, summaries):
        N = len(contents_tokenized)
        maskeds, unmaskeds = [], []
        summ_toks = []
        T1 = time.time()
        for content_tokenized, content, summary in zip(contents_tokenized, contents, summaries):
            masked = self.mask_text(content_tokenized, content) # .decode()
    ...
    def train_batch(self, contents, summaries):
        ...
        input_ids, is_masked, labels = self.build_io(contents_tokenized, contents, summaries)
    ...
    def score(self, summaries, contents, bodies_tokenized, lengths=None, extra=None):
        ...
        # self.model.eval()
        with torch.no_grad():
            input_ids_w, is_masked_w, labels_w = self.build_io(contents_tokenized, contents, summaries)
            ...
            if extra is not None:
                ...
                input_ids_wo, is_masked_wo, labels_wo = self.build_io(contents_tokenized, contents, [""] * len(contents_tokenized))
